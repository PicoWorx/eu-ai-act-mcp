...
function obligationsSummaryForHighRisk(role) {
    ...
    if (role === "deployer") {
        return "Deployer obligations: use per instructions (Art. 26(1)), human oversight by competent persons (Art. 26(2)), input data relevance (Art. 26(4)), monitor and report incidents (Art. 26(5)), DPIA where required (Art. 26(9)), inform affected persons (Art. 26(7)(11)), and perform a FRIA where Art. 27 applies, including public-law/public-service deployers and Annex III(5)(b)/(c) credit and life/health-insurance use cases.";
    }
...
function classifyFromSignals(input) {
    ...
    if (s.performs_social_scoring || s.performs_social_scoring_by_public_authority) {
        matched.push(s.performs_social_scoring
    ...
    if (s.uses_biometrics) {
        ...
        matched.push("uses_biometrics → Annex III(1) Biometrics");
        return {
            ...buildBase({
