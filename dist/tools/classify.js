...
function isSolePurposeBiometricVerification(text) {
...
function isTravelDocumentVerificationOnly(text) {
    const normalized = text.toLowerCase();
...
function isNegatedCriminalProfiling(text) {
    const normalized = text.toLowerCase();
...
function isGenericAggregatedCrimeAnalytics(text) {
    const normalized = text.toLowerCase();
    ...
    if (s.is_safety_component_of_regulated_product) {
        ...
        if (!s.domain || DOMAIN_TO_ANNEX_III[s.domain] === undefined) {
            ...
            return insufficientFromSignals({
                ...
                obligationsSummary: s.requires_third_party_conformity_assessment === false
                ...
                nextQuestions: s.requires_third_party_conformity_assessment === undefined
...
function classifyFromText(input) {
    ...
    let prohibitedHit = bestStrongHit(combined, prohibitedPractices);
    ...
    let annexHit = bestStrongHit(combined, annexIIICategories);
    if (annexHit &&
