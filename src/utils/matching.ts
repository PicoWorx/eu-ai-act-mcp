...
export function calculateKeywordOverlap(text: string, keywords: string[]): number {
  ...
  const matches = keywords.filter(kw => {
    ...
    return stems.some(stem =>
      textWords.some(tw => tw.startsWith(stem) || stem.startsWith(tw))
    );
  });
...
function getStemVariants(word: string): string[] {
  ...
  if (word.endsWith("s") && !word.endsWith("ss")) variants.push(word.slice(0, -1));
...
export function findBestMatch<T extends Record<string, any>>(
  ...
  for (const item of items) {
    ...
    for (const word of words) {
      const wordStems = [word, ...getStemVariants(word)];
      const matched = wordStems.some(stem =>
        itemWords.some(iw => iw.startsWith(stem) || stem.startsWith(iw))
      );
