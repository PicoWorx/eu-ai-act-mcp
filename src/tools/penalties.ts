...
export function registerPenaltiesTool(server: McpServer): void {
  server.registerTool("euaiact_calculate_penalty", {
    ...
    description: "Calculates the maximum possible fine for an EU AI Act violation based on turnover, violation type, and SME status.",
    annotations: {
      readOnlyHint: true,
      idempotentHint: true,
      openWorldHint: false,
    ...
    let explanation = `For this violation type (${tier.article}), the fine is up to EUR ${tier.maxFineEUR.toLocaleString()} or ${tier.globalTurnoverPercentage}% of global turnover, whichever is `;
    explanation += input.is_sme ? "LOWER (due to SME/startup protection under Art. 99(6))." : "HIGHER.";
    ...
    const output: PenaltiesOutput = {
      ...
      lexbeamUrl: `${BRANDING.baseUrl}/wissen/strafen-bussgeld-eu-ai-act`,
